...
/**
 * These tests TRUNCATE decision_log. That is fine against a local or CI
 * database and catastrophic against one somebody else is using - a shared
 * hosted dev database would have one person's test run wipe another's ledger
 * mid-build.
 *
 * So: local hosts run freely; anything else needs a deliberate opt-in.
 */
...
const canRun = Boolean(url) && (isLocal(url!) || optedIn);

if (url && !canRun) {
  console.warn(
    `[ledger.test] skipping - DATABASE_URL points at a non-local host and these tests wipe decision_log.
` +
      `  If it is YOUR OWN database and nobody else uses it: ALLOW_DESTRUCTIVE_TESTS=1 pnpm test`,
  );
}

describe.skipIf(!canRun)("decision ledger", () => {
  beforeEach(async () => {
